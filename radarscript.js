            ...
            data.forEach(function(feature) {
                try {
                    feature.geometry.coordinates.forEach(function(object){
                        var polygon = L.polygon(reverseSubarrays(object[0]), { 'pane': 'underRadar', 'color': feature.properties.stroke, 'fillOpacity': 0, 'className': 'noselect' }).addTo(map);
                    });
                } catch {
                    feature.geometry.coordinates.forEach(function(object){
                        var polygon = L.polygon(reverseSubarrays(object), { 'pane': 'underRadar', 'color': feature.properties.stroke, 'fillOpacity': 0, 'className': 'noselect' }).addTo(map);
                    });
                }
            });
        }
